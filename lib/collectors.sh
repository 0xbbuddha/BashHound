...
# Nettoyage des fichiers temporaires géré par le script principal après export
...
collect_users() {
    ...
    while IFS= read -r line; do
        if [ -n "$line" ] && [[ "$line" =~ ^308 ]]; then
            ...
            # Exclure les comptes ordinateur (UAC 0x1000 = WORKSTATION_TRUST_ACCOUNT)
            if [ -n "$uac" ] && (( uac & 0x1000 )); then
                continue
            fi
            # Exclure les objets dans OU=Domain Controllers (collectés comme computers)
...
collect_containers() {
    ...
    # Collecte depuis le Domain NC (DC=domain,DC=tld) - recherche en sous-arbre
    ...
    # Collecte depuis le Configuration NC (containers utiles pour la topologie)
